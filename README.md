...
Product images live in `images/`. The file `data/image_manifest.csv` maps each workbook product number to a hero image and an optional pipe-separated gallery.

Example:

```csv
No.,Image Path,Gallery Paths
2,images/02_cardipulse_pfa_system.jpg,images/02_cardipulse_pfa_system.jpg|images/02_cardipulse_pfa_brochure.webp
```

To add another image, place it in `images/` and append its relative path to `Gallery Paths`. The first path is used as the product-card hero image. Uploaded Excel or CSV datasets are also matched to this manifest by `No.` when available, then by product name.

The current image set covers products 1–3 and 5–12. Product 4 uses the app's fallback until an image is added.
